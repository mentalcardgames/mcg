...
pub fn get_available_characters(ctx: &Context) -> HashSet<char> {
    ...
    ctx.fonts(|f| {
        f.lock().fonts.font(&font_id).characters().iter()
...
pub fn get_available_emojis(ctx: &Context) -> Vec<char> {
    const EMOJI_RANGES: &[(u32, u32)] = &[
        (0x1F600, 0x1F64F), (0x2600, 0x26FF), (0x1F464, 0x1F49F), (0x1F44A, 0x1F450), (0x2700, 0x27BF), (0x1F300, 0x1F5FF),
    ];
    let available_chars = get_available_characters(ctx);
    let mut emojis = Vec::new();
    for (start, end) in EMOJI_RANGES { for code_point in *start..=*end { if let Some(emoji_char) = char::from_u32(code_point) { if available_chars.contains(&emoji_char) { emojis.push(emoji_char); } } } }
    emojis
}

pub fn emoji_hash(data: &[u8], ctx: &Context) -> String {
    let emojis = get_available_emojis(ctx);
    if emojis.is_empty() { return "???".to_string(); }
    let mut hasher = Sha256::new(); hasher.update(data); let result = hasher.finalize();
