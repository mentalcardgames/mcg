...
* **Visual Diagrams:** Renders your game logic into `.svg` diagrams for easy documentation and debugging.
...
3.  The extension automatically maintains three outputs in your workspace:
    ...
    * `*.svg`: A high-resolution visual map of the game's Finite State Machine.
...
* `cgdsl.outputDirectory`: Specify a custom subfolder where `.svg` and `.json` files should be saved.
...
* Automatic export of SVG, DOT, and JSON.
