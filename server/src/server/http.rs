...
// crate::server::state to operate on the shared AppState and return the same
...
pub async fn join_handler(State(state): State<AppState>, Json(cm): Json<ClientMsg>) -> impl IntoResponse {
    match cm {
        ClientMsg::Join { name } => {
            ...
            // Also send initial state to broadcaster (server printing/bookkeeping is done elsewhere)
            crate::server::broadcast_state(&state, 0).await;
...
pub async fn next_hand_handler(State(state): State<AppState>) -> impl IntoResponse {
    if let Err(e) = crate::server::start_new_hand_and_print(&state, 0).await {
...
pub async fn reset_handler(State(state): State<AppState>, Json(cm): Json<ClientMsg>) -> impl IntoResponse {
    match cm {
        ClientMsg::ResetGame { bots } => {
            ...
            let name = "HTTP";
            if let Err(e) = crate::server::reset_game_with_bots(&state, name, bots, 0).await {
