...
// fn id_not_initialized(value: &TypedVars, id: &SID) -> Result<(), SymbolError> {
//   if value.iter().map(|(s, _)| s.id.clone()).collect::<Vec<String>>().contains(&id.node) {
//     return Ok(())
//   }

//   return Err(SymbolError::NotInitialized { var: Var::from(id.clone()) })
// }

// fn id_used(value: &mut TypedVars, id: &SID, ty: GameType) -> Result<(), SymbolError> {
//   if !value.iter().map(|(s, _)| s.id.clone()).collect::<Vec<String>>().contains(&id.node) {
//     let var = Var {
//       id: id.node.clone(),
//       span: id.span.clone(),
...
//   return Err(SymbolError::SymbolUsed { var: Var::from(id.clone()) })
// }
...
//         return Err(SymbolError::InitializationMightNotHappen { span: self.span.clone() })
//       }
//     }
