...
| Take from a chosen player | Go Fish ask | ✅ | ✅ | ✅ | `deal Hand where Rank is "X" of any …` — source `any` prompts `ChoosePlayer` (engine: `SourcePlayerAny`) |
| Turn order fixed/random | any | ✅ | ✅ | ✅ | `turnorder all` / `turnorder all random` |
| Teams | Bridge, Doppelkopf | ✅ | ✅ | ✅ | `team T1 with all`; `location X on T:T1` = one shared pile per team |
...
| Only current player acts | most | ✅ | ✅ | ✅ | Enforced by input validation (I-23); eliminated players are never prompted (I-24) |

## 5. Actions & moves

| Mechanic | Example games | DSL | Parser | Engine | Notes / what's missing |
|---|---|---|---|---|---|
| Draw a fixed count | most | ✅ | ✅ | ✅ | `deal 2 from Deck …` (automatic, from the top) |
| Draw a computed count | memory-driven games | ✅ | ✅ | ✅ | `deal (&I:M of current) from Deck …` (quantities evaluated against the live state) |
...
| Play a *chosen* card | Crazy Eights | ✅ | ✅ | ✅ | `move 1 from Hand of current …` — pick exactly one |
...
| Bid / auction | Bridge, Poker | ✅ | ✅ | ✅ | `bid <qty> on <memory> of <owner>` — the numeric input prompt; plain `bid` errors; `demand` still stubs (D-7) |
...
| Per-player aggregation in conditions | "all players have ≥3 cards" | ⚠️ | ⚠️ | ⚠️ | Per-player memory aggregates work; general iteration over players in one predicate is not expressible — DSL design |
...
| Winner by position | party games | ✅ | ✅ | ✅ | Turn-order index (lower = earlier); players absent from `turn_order` are excluded |
...
|---|---|---|
| ~~Numeric input prompt~~ | **Done** — `bid <qty> on <memory> of <owner>` (`InputType::Number`); `any` in pure int slots still needs grammar | §8 |
